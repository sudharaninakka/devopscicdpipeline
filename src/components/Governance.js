...
const Governance = (props) => {
  ...
  useEffect(() => {
    ...
    try {
      ...
      var newData = JSON.parse(localStorage.getItem("governance"));
      if (newData) {
  ...
  useEffect(() => {
    ...
    localStorage.setItem("governance", JSON.stringify(foundation));

    allData.Governance.map((ele, idx) => {
      if (idx === 0) {
        ele["answer"] = role1ans;
    ...
    if (role1ans && role2ans && role3ans && role4ans && role5ans && role6ans) {
      localStorage.setItem("govDone", JSON.stringify(true));
    } else {
      localStorage.setItem("govDone", JSON.stringify(false));
    }
          ...
          <div className="ro-mid">
            ...
            <div className="ro-mid1">
              <div className="ro-mid1-head">
                ...
                <Link to="/governance">
                  <p className={`${govComplete ? `completed` : ``} current`}>
                    Governance
                  </p>
                </Link>
                <Link to="/value">
              ...
              <div className="ro-mid1-p">
                {allData.Governance.map((data, idx) => {
                  var inputData = "";
                  var seeing = "";
