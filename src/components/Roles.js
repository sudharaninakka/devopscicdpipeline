...
const Roles = (props) => {
  ...
  useEffect(() => {
    ...
    try {
      ...
      var newData = JSON.parse(localStorage.getItem("roles"));
      if (newData) {
  ...
  useEffect(() => {
    ...
    localStorage.setItem("roles", JSON.stringify(foundation));

    allData.Roles.map((ele, idx) => {
      if (idx === 0) {
        ele["answer"] = role1ans;
    ...
    if (role1ans && role2ans && role3ans && role4ans && role5ans) {
      localStorage.setItem("roleDone", JSON.stringify(true));
    } else {
      localStorage.setItem("roleDone", JSON.stringify(false));
    }
          ...
          <div className="ro-mid">
            ...
            <div className="ro-mid1">
              <div className="ro-mid1-head">
                ...
                <Link to="/roles">
                  <p className={`${roleComplete ? `completed` : ``} current `}>
