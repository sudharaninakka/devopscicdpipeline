...
const Budget = (props) => {
  ...
  useEffect(() => {
    ...
    try {
      ...
      var newData = JSON.parse(localStorage.getItem("budget"));
      if (newData) {
  ...
  useEffect(() => {
    ...
    localStorage.setItem("budget", JSON.stringify(foundation));

    allData.Budget.map((ele, idx) => {
      if (idx === 0) {
        ele["answer"] = role1ans;
    ...
    if (role1ans && role2ans && role3ans && role4ans && role5ans && role6ans) {
      localStorage.setItem("budgetDone", JSON.stringify(true));
    } else {
      localStorage.setItem("budgetDone", JSON.stringify(false));
    }
          ...
          <div className="ro-mid">
            ...
            <div className="ro-mid1">
              <div className="ro-mid1-head">
                ...
                <Link to="/budget">
                  <p
                    className={`${budgetComplete ? `completed` : ``}  current`}
                  >
                    Budget
                  </p>
                </Link>
