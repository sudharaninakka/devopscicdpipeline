...
const Purchase = (props) => {
  ...
  useEffect(() => {
    ...
    try {
      ...
      var newData = JSON.parse(localStorage.getItem("purchase"));
      if (newData) {
  ...
  useEffect(() => {
    ...
    localStorage.setItem("purchase", JSON.stringify(foundation));

    allData.Purchase.map((ele, idx) => {
      if (idx === 0) {
        ele["answer"] = role1ans;
    ...
    if (role1ans && role2ans && role3ans && role4ans && role5ans && role6ans) {
      localStorage.setItem("purDone", JSON.stringify(true));
    } else {
      localStorage.setItem("purDone", JSON.stringify(false));
    }
          ...
          <div className="ro-mid">
            ...
            <div className="ro-mid1">
              <div className="ro-mid1-head">
                ...
                <Link to="/purchase">
                  <p className={`${purComplete ? `completed` : ``} current`}>
                    Purchase
                  </p>
                </Link>
                <Link to="/process">
