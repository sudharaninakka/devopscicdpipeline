...
const Architecture = (props) => {
  ...
  useEffect(() => {
    ...
    try {
      ...
      var newData = JSON.parse(localStorage.getItem("archi"));
      if (newData) {
  ...
  useEffect(() => {
    ...
    localStorage.setItem("archi", JSON.stringify(foundation));

    allData.Architecture.map((ele, idx) => {
      if (idx === 0) {
        ele["answer"] = role1ans;
    ...
    if (role1ans && role2ans && role3ans && role4ans && role5ans) {
      localStorage.setItem("archiDone", JSON.stringify(true));
    } else {
      localStorage.setItem("archiDone", JSON.stringify(false));
    }
          ...
          <div className="ro-mid">
            ...
            <div className="ro-mid1">
              <div className="ro-mid1-head">
                ...
                <Link to="/archi">
                  <p className={`${archiComplete ? `completed` : ``} current`}>
                    Architecture
                  </p>
                </Link>
                <Link to="/visi">
              ...
              <div className="ro-mid1-p">
                {allData.Architecture.map((data, idx) => {
                  var inputData = "";
                  var seeing = "";
