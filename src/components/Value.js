...
const Value = (props) => {
  ...
  useEffect(() => {
    ...
    try {
      ...
      var newData = JSON.parse(localStorage.getItem("value"));
      if (newData) {
  ...
  useEffect(() => {
    ...
    localStorage.setItem("value", JSON.stringify(foundation));

    allData.Value.map((ele, idx) => {
      if (idx === 0) {
        ele["answer"] = role1ans;
    ...
    if (role1ans && role2ans && role3ans && role4ans && role5ans && role6ans) {
      localStorage.setItem("valueDone", JSON.stringify(true));
    } else {
      localStorage.setItem("valueDone", JSON.stringify(false));
    }
          ...
          <div className="ro-mid">
            ...
            <div className="ro-mid1">
              <div className="ro-mid1-head">
                ...
                <Link to="/value">
                  <p className={`${valueComplete ? `completed` : ``} current`}>
                    Value
                  </p>
                </Link>
                <Link to="/right">
