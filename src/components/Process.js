...
const Process = (props) => {
  ...
  useEffect(() => {
    ...
    try {
      ...
      var newData = JSON.parse(localStorage.getItem("process"));
      if (newData) {
  ...
  useEffect(() => {
    ...
    localStorage.setItem("process", JSON.stringify(foundation));

    allData.Process.map((ele, idx) => {
      if (idx === 0) {
        ele["answer"] = role1ans;
    ...
    if (role1ans && role2ans && role3ans && role4ans && role5ans) {
      localStorage.setItem("processDone", JSON.stringify(true));
    } else {
      localStorage.setItem("processDone", JSON.stringify(false));
    }
        ...
        <div className="ro-full">
          ...
          <div className="ro-mid">
            ...
            <div className="ro-mid1">
              <div className="ro-mid1-head">
                ...
                <Link to="/right">
                  <p className={`${rightComplete ? `completed` : ``} `}>
                    Right - Sizing
                  </p>
                </Link>
                <Link to="/purchase">
                ...
                <Link to="/process">
                  <p
                    className={`${processComplete ? `completed` : ``} current`}
                  >
                    Process
                  </p>
                </Link>
          ...
          <div className="bot-btn">
            ...
            {fullDone ? (
              <button
                onClick={() => onClick("displayBasic")}
                className="bottombtn next last-btn"
              >
                Review <BsChevronRight />
              </button>
            ) : (
              <button
                // onClick={() => onClick("displayBasic")}
                className="bottombtn next last-btn pending-review"
              >
                Review <BsChevronRight />
              </button>
            )}
